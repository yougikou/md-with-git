...
> **リリース状況**：このリポジトリは Vite Viewer としてそのまま実行でき、ホストおよびレンダラーの実装例を含みます。`@md-with-git/viewer` の安定した import 可能な `DocsViewer` エントリは現在整備中です。公開されるまでは、このリポジトリをアプリケーションまたはソース統合として使用し、以下のパッケージ import を本番利用可能なものとして扱わないでください。
