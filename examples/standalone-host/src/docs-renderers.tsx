...
function ChangeHistoryTable({ children }: { children: ReactNode }) {
  return <div className="change-history-table-scroll"><table className="change-history-table">{children}</table></div>;
}

export function ChangeHistoryRenderer({ value }: YamlBlockRendererProps) {
  const entries = normalizeHistory(value);
  if (!entries.length) return <div className="yaml-renderer-error">change-history 需要包含有效的 entries（version、date、summary）。</div>;
  ...
  return <section className="change-history" aria-label="变更履历"><div className="change-history-heading"><span className="eyebrow">CHANGE HISTORY</span><span>{entries.length} 个版本</span></div><ChangeHistoryTable><thead><tr><th>版本</th><th>日期</th><th>变更</th></tr></thead><tbody>{rows(recent)}</tbody></ChangeHistoryTable>{older.length > 0 && <details><summary>查看更早的 {older.length} 个变更</summary><ChangeHistoryTable><thead><tr><th>版本</th><th>日期</th><th>变更</th></tr></thead><tbody>{rows(older)}</tbody></ChangeHistoryTable></details>}</section>;
}
